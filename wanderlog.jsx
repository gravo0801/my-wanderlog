...
const EXP_CATS = [
  ...
  {id:"shopping",   label:"?쇳븨",   icon:"?썚截?, color:"#FBD38D"},
  {id:"other",      label:"湲고?",   icon:"?뮩", color:"#D6BCFA"},
];
const PAYMENT_METHODS = [
  {id:"card",   label:"?뮩 ?좎슜移대뱶"},
  {id:"cash",   label:"?뮫 ?꾧툑"},
  {id:"travel", label:"?お ?몃옒釉붿껜?ъ뭅??},
];
const TRANSPORT_MODES = [
  ...
  {id:"flight",   label:"??났",     icon:"?덌툘", maps:null},
  {id:"boat",     label:"?좊컯",     icon:"?댐툘", maps:"transit"},
];
...
function placeIcon(type="") {
  ...
  if (["restaurant","cafe","fast_food","bar","pub","izakaya","ramen","sushi","food","eatery","bistro","diner"].some(x=>t.includes(x))) return "?띂截?;
  if (["museum","attraction","viewpoint","monument","castle","temple","shrine","cathedral","church","ruins","gallery"].some(x=>t.includes(x))) return "?룢截?;
  ...
  if (["beach","bay","coast","sea"].some(x=>t.includes(x))) return "?룚截?;
  if (["mall","shop","market","shopping"].some(x=>t.includes(x))) return "?썚截?;
      ...
      <div className={`right-panel ${screen!=="home"?"active-mobile":"hidden-mobile"}`}>
        ...
        {screen==="home" && (
          <div className="hidden-mobile" style={{height:"100%",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:14,color:"#B0BEC5"}}>
            <div style={{fontSize:52,opacity:.35}}>?덌툘</div>
    ...
    <div style={{height:"100%",overflowY:"auto"}}>
      ...
      {stats.days > 0 && (
        <div style={{display:"flex",gap:0,borderBottom:"1px solid #F0EDE8",background:"#FFF"}}>
          {[{v:trips.length,l:"?ы뻾",i:"?덌툘"},{v:stats.days,l:"湲곕줉??,i:"?뱟"},{v:stats.places,l:"諛⑸Ц吏",i:"?뱧"}].map((x,idx)=>(
              ...
              return (
                <div key={t.id} className="tbtn" style={{borderRadius:20,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,.07)",border:"1px solid rgba(0,0,0,.04)",background:"#FFF"}} onClick={()=>onSelect(t)}>
                  <div style={{height:155,position:"relative",background:t.coverImage?`url(${t.coverImage}) center/cover`:t.gradient}}>
                    ...
                    <div style={{position:"absolute",top:12,right:12,fontSize:28,filter:"drop-shadow(0 2px 6px rgba(0,0,0,.4))"}}>{t.flag||"?덌툘"}</div>
    ...
    <div style={{height:"100%",overflowY:"auto",paddingBottom:80}}>
      ...
      <div style={{height:260,position:"relative",background:trip.coverImage?`url(${trip.coverImage}) center/cover`:trip.gradient}}>
        ...
        <div style={{position:"absolute",bottom:20,left:22,right:22,zIndex:5}}>
          <div style={{fontSize:44,marginBottom:6,filter:"drop-shadow(0 3px 8px rgba(0,0,0,.4))"}}>{trip.flag||"?덌툘"}</div>
      ...
      <div style={{padding:"20px 20px 0"}}>
        ...
        {tab==="map" && (
          <div className="fade-up" style={{textAlign:"center",padding:"40px 16px"}}>
            <div style={{fontSize:52,marginBottom:14}}>?뿺截?/div>
  ...
  return (
    <div style={{height:"100%",overflowY:"auto",paddingBottom:90}}>
      ...
      <div style={{padding:"20px 20px 0"}}>
        ...
        {/* Diary */}
        <div style={S.secBox}>
          <div style={S.secTitle}>?랃툘 ?ы뻾 ?명듃</div>
...
function NewTripModal({ onClose, onCreate }) {
  ...
  const create = () => {
    if (!valid) return;
    onCreate({id:uid(),...form,flag:form.flag||"?덌툘",days:dateRange(form.startDate,form.endDate).map(date=>({date,waypoints:[newWaypoint()],diary:"",photos:[],expenses:[]}))});
  };
