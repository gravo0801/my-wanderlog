...
/* ── Storage (localStorage로 변경 - Vercel 호환) ───────────────────────────── */
const STORAGE_KEY = "wl_trips";

function storageSave(trips) {
  try {
...
const TRANSPORT = [
  ...
  {id:"bicycling",label:"자전거",  icon:"🚲", maps:"bicycling"},
  {id:"train",    label:"기차",    icon:"🚄", maps:"transit"},
  {id:"flight",   label:"항공",    icon:"✈️", maps:"transit"},
...
async function callClaude(prompt, maxTokens=700) {
  if (!CLAUDE_API_KEY) {
    console.error("Claude API key is missing (VITE_CLAUDE_API_KEY)");
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      console.error("Claude API failed:", res.status, await res.text());
      return null;
    }
...
const RATE_RANGES = {
  USD:[1100,1700], EUR:[1200,1900], JPY:[6,13], GBP:[1400,2200],
  CNY:[140,230], THB:[25,55], VND:[0.03,0.08], SGD:[850,1200],
  AUD:[700,1100], TWD:[35,55], HKD:[140,210]
};
      ...
      <div style={{background:"rgba(212,168,83,.06)",border:"1px solid rgba(212,168,83,.18)",borderRadius:18,padding:"20px 18px 16px",marginBottom:16,textAlign:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:7,marginTop:10}}>
          <button onClick={loadRates} disabled={loading}
            style={{background:"rgba(212,168,83,.1)",border:"1px solid rgba(212,168,83,.25)",color:"#D4A853",padding:"4px 12px",borderRadius:7,cursor:"pointer",fontSize:11,opacity:loading?0.5:1}}>
            {loading?"조회중...":"🔄 환율 새로고침"}
          </button>
